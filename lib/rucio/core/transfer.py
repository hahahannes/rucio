    ...
    for req_id, rule_id, scope, name, md5, adler32, bytes, activity, attributes, previous_attempt_id, dest_rse_id, source_rse_id, rse, deterministic, rse_type, path, retry_count, src_url, ranking, link_ranking in req_sources:
        ...
        try:
            ...
            if req_id not in transfers:
                ...
                # get external_host
                external_hosts_from_config = config_get('conveyor', 'external_hosts')
                external_hosts_list = []
                if external_hosts_from_config:
                    if ',' in external_hosts_from_config:
                        external_hosts_list = external_hosts_from_config.split(',')
                    else:
                        external_hosts_list = [external_hosts_from_config]
                else:
                    ...
                    if not fts_hosts:
                        ...
                        external_hosts_list = fts_hosts.split(",")
                ...
                external_host = external_hosts_list[0]
                if retry_other_fts:
                    external_host = external_hosts_list[retry_count % len(external_hosts_list)]
