...
@transactional_session
def release_waiting_requests_per_free_volumn(rse, rse_id=None, max_transfer_volumn=None, session=None):
    """
    Release waiting requests if they fit in available transfer volumn. If the DID of a request is attached to a dataset, the volumn will be checked for the whole dataset as all requests related to this dataset will be released.
    ...
    sum_volumn_active_subquery = None
    if dialect == 'mysql':
        sum_volumn_active_subquery = session.query(func.ifnull(func.sum(models.Request.bytes), 0).label('sum_bytes'))\
        ...
        sum_volumn_active_subquery = session.query(func.coalesce(func.sum(models.Request.bytes), 0).label('sum_bytes'))\
                                            .filter(and_(or_(models.Request.state == RequestState.SUBMITTED, models.Request.state == RequestState.QUEUED),
                                                         models.Request.dest_rse_id == rse_id))
    elif dialect == 'oracle':
        sum_volumn_active_subquery = session.query(func.nvl(func.sum(models.Request.bytes), 0).label('sum_bytes'))\
                                            .filter(and_(or_(models.Request.state == RequestState.SUBMITTED, models.Request.state == RequestState.QUEUED),
                                                         models.Request.dest_rse_id == rse_id))
    sum_volumn_active_subquery = sum_volumn_active_subquery.subquery()
    ...
    cumulated_volumn_subquery = session.query(grouped_requests_subquery.c.name,
                                              grouped_requests_subquery.c.scope,
                                              func.sum(grouped_requests_subquery.c.volumn).over(order_by=grouped_requests_subquery.c.oldest_requested_at).label('cum_volumn'))\
                                       .filter(grouped_requests_subquery.c.volumn <= max_transfer_volumn - sum_volumn_active_subquery.c.sum_bytes)\
                                       .cte()

    cumulated_volumn_subquery = session.query(filtered_requests_subquery.c.id)\
                                       .join(cumulated_volumn_subquery, and_(filtered_requests_subquery.c.dataset_name == cumulated_volumn_subquery.c.name, filtered_requests_subquery.c.dataset_scope == cumulated_volumn_subquery.c.scope))\
                                       .filter(cumulated_volumn_subquery.c.cum_volumn <= max_transfer_volumn - sum_volumn_active_subquery.c.sum_bytes)\
                                       .subquery()

    statement = update(models.Request).where(models.Request.id.in_(cumulated_volumn_subquery)).values(state=RequestState.QUEUED)
...
def create_base_query_grouped_fifo(rse, rse_id=None, filter_by_rse='destination', session=None):
    ...
    filtered_requests_subquery = filtered_requests_subquery.filter(models.Request.state == RequestState.WAITING).cte()
    ...
    grouped_requests_subquery = session.query(func.sum(combined_attached_unattached_requests.c.bytes).label('volumn'),
...
@transactional_session
def release_waiting_requests_grouped_fifo(dest_rse, dest_rse_id=None, count=None, session=None):
    """
    Release waiting requests. Transfer requests that were requested first, get released first (FIFO).
    ...
    :param rse:              The RSE name.
    :param rse_id:           The transfer destination RSE id.
    :param count:            The count to be released. If None, release all waiting requests.
    :param session:          The database session.
    """
    if not dest_rse_id:
        dest_rse_id = get_rse_id(rse=dest_rse, session=session)

    transfer_limits = None
    amount_updated_requests = 0
    grouped_requests_subquery, filtered_requests_subquery = create_base_query_grouped_fifo(dest_rse, dest_rse_id, filter_by_rse='destination', session=session)
    ...
    # release requests where the whole datasets volumn fits in the available volumn space
    all_activities = 'all_activities'
    transfer_limits = get_rse_transfer_limits(dest_rse, all_activities, dest_rse_id, session=session)[all_activities][dest_rse_id]
    max_transfer_volumn = transfer_limits.get('max_volumn', 0)
    amount_updated_requests += release_waiting_requests_per_free_volumn(dest_rse, dest_rse_id, max_transfer_volumn=max_transfer_volumn, session=session)
