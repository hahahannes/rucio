...
from rucio.core.config import set, remove_option, has_option
...
class TestThrottlerGroupedFIFO(object):
    ...
    def setUpClass(cls):
        ...
        if has_option('throttler', 'mode', use_cache=False, session=cls.db_session):
            remove_option('throttler', 'mode', session=cls.db_session)
        set('throttler', 'mode', 'DEST_PER_ALL_ACT', session=cls.db_session)
    ...
    def test_throttler_grouped_fifo_all(self):
        ...
        set_rse_transfer_limits(rse=self.dest_rse, max_transfers=1, activity=self.all_activities, strategy='grouped_fifo', session=self.db_session)
        name1 = generate_uuid()
        name2 = generate_uuid()
        name3 = generate_uuid()
        name4 = generate_uuid()
    ...
    def test_throttler_grouped_fifo_subset(self):
        ...
        set_rse_transfer_limits(self.dest_rse, self.all_activities, volume=10, max_transfers=1, deadline=1, strategy='grouped_fifo', session=self.db_session)
        name1 = generate_uuid()
        name2 = generate_uuid()
        name3 = generate_uuid()
        name4 = generate_uuid()
...
class TestThrottlerFIFO(object):
    ...
    def setUpClass(cls):
        ...
        if has_option('throttler', 'mode', use_cache=False, session=cls.db_session):
            remove_option('throttler', 'mode', session=cls.db_session)
        set('throttler', 'mode', 'DEST_PER_ACT', session=cls.db_session)
    ...
    def test_throttler_fifo_release_all(self):
        ...
        set_rse_transfer_limits(rse=self.dest_rse, max_transfers=1, activity=self.all_activities, strategy='fifo', session=self.db_session)
        name1 = generate_uuid()
        name2 = generate_uuid()
        add_replica(self.source_rse, self.scope, name1, 1, self.account, session=self.db_session)
        add_replica(self.source_rse, self.scope, name2, 1, self.account, session=self.db_session)
        ...
        name1 = generate_uuid()
        add_replica(self.source_rse, self.scope, name1, 1, self.account, session=self.db_session)
        set_rse_transfer_limits(rse=self.dest_rse, activity=self.user_activity, max_transfers=3, strategy='fifo', session=self.db_session)
    ...
    def test_throttler_fifo_release_nothing(self):
        ...
        # more than 80% of the transfer limit are already used -> release nothing
        set_rse_transfer_limits(rse=self.dest_rse, max_transfers=1, activity=self.user_activity, strategy='fifo', session=self.db_session)
        request = models.Request(dest_rse_id=self.dest_rse_id, bytes=2, activity=self.user_activity, state=constants.RequestState.SUBMITTED)
        request.save(session=self.db_session)
        name1 = generate_uuid()
        name2 = generate_uuid()
    ...
    def test_throttler_fifo_release_subset(self):
        ...
        set_rse_transfer_limits(activity=self.user_activity, max_transfers=1, strategy='fifo', rse=self.dest_rse, session=self.db_session)
        name1 = generate_uuid()
        name2 = generate_uuid()
        add_replica(self.source_rse, self.scope, name1, 1, self.account, session=self.db_session)
        add_replica(self.source_rse, self.scope, name2, 1, self.account, session=self.db_session)
...
class TestThrottlerFIFOSRCACT(object):
    ...
    def setUpClass(cls):
        ...
        if has_option('throttler', 'mode', use_cache=False, session=cls.db_session):
            remove_option('throttler', 'mode', session=cls.db_session)
        set('throttler', 'mode', 'SRC_PER_ACT', session=cls.db_session)
    ...
    def test_throttler_fifo_release_subset(self):
        ...
        set_rse_transfer_limits(activity=self.user_activity, max_transfers=1, strategy='fifo', rse=self.source_rse, session=self.db_session)
        set_rse_transfer_limits(activity=self.user_activity2, max_transfers=0, strategy='fifo', rse=self.source_rse, session=self.db_session)
        name1 = generate_uuid()
        name2 = generate_uuid()
        name3 = generate_uuid()
        add_replica(self.source_rse, self.scope, name1, 1, self.account, session=self.db_session)
        ...
        request = get_request_by_did(self.scope, name1, self.dest_rse)
        assert_equal(request['state'], constants.RequestState.QUEUED)
        request2 = get_request_by_did(self.scope, name2, self.dest_rse2)
        assert_equal(request2['state'], constants.RequestState.WAITING)
        request3 = get_request_by_did(self.scope, name3, self.dest_rse3)
...
class TestThrottlerFIFOSRCALLACT(object):
    ...
    def setUpClass(cls):
        ...
        if has_option('throttler', 'mode', use_cache=False, session=cls.db_session):
            remove_option('throttler', 'mode', session=cls.db_session)
        set('throttler', 'mode', 'SRC_PER_ALL_ACT', session=cls.db_session)
    ...
    def test_throttler_fifo_release_subset(self):
        ...
        # two waiting requests and no active requests but threshold is 1 -> release only 1 request
        set_rse_transfer_limits(activity=self.all_activities, max_transfers=1, strategy='fifo', rse=self.source_rse, session=self.db_session)
        name1 = generate_uuid()
        name2 = generate_uuid()
        add_replica(self.source_rse, self.scope, name1, 1, self.account, session=self.db_session)
        add_replica(self.source_rse, self.scope, name2, 1, self.account, session=self.db_session)
...
class TestThrottlerFIFODESTALLACT(object):
    ...
    def test_throttler_fifo_release_subset(self):
        ...
        set_rse_transfer_limits(activity=self.all_activities, max_transfers=1, strategy='fifo', rse=self.dest_rse, session=self.db_session)
        set_rse_transfer_limits(activity=self.all_activities, max_transfers=0, strategy='fifo', rse=self.dest_rse2, session=self.db_session)
        name1 = generate_uuid()
        name2 = generate_uuid()
        name3 = generate_uuid()
        add_replica(self.source_rse, self.scope, name1, 1, self.account, session=self.db_session)
        add_replica(self.source_rse2, self.scope, name2, 1, self.account, session=self.db_session)
        add_replica(self.source_rse, self.scope, name3, 1, self.account, session=self.db_session)
        requests = [{
            'dest_rse_id': self.dest_rse_id,
            'source_rse_id': self.source_rse_id,
        ...
        request2 = get_request_by_did(self.scope, name2, self.dest_rse)
        assert_equal(request2['state'], constants.RequestState.WAITING)
        request3 = get_request_by_did(self.scope, name3, self.dest_rse2)
        assert_equal(request3['state'], constants.RequestState.WAITING)
...
class TestThrottlerGroupedFIFOSRCALLACT(object):
    ...
    def test_throttler_grouped_fifo_subset(self):
        ...
        set_rse_transfer_limits(self.source_rse, self.all_activities, volume=10, max_transfers=1, deadline=0, strategy='grouped_fifo', session=self.db_session)
        name1 = generate_uuid()
        name2 = generate_uuid()
        name3 = generate_uuid()
        name4 = generate_uuid()
        ...
        assert_equal(request_1['state'], constants.RequestState.WAITING)
        request_2 = get_request_by_did(self.scope, name2, self.dest_rse_2, session=self.db_session)
        assert_equal(request_2['state'], constants.RequestState.WAITING)
        request_3 = get_request_by_did(self.scope, name3, self.dest_rse, session=self.db_session)
        assert_equal(request_3['state'], constants.RequestState.WAITING)
        request_4 = get_request_by_did(self.scope, name4, self.dest_rse_2, session=self.db_session)
        assert_equal(request_4['state'], constants.RequestState.WAITING)
        ...
        assert_equal(request_1['state'], constants.RequestState.QUEUED)
        # released because the DID is attached to the same dataset
        request_2 = get_request_by_did(self.scope, name2, self.dest_rse_2)
        assert_equal(request_2['state'], constants.RequestState.QUEUED)
        # still waiting, volume check is only working for destination RSEs (writing)
        request_3 = get_request_by_did(self.scope, name3, self.dest_rse)
        assert_equal(request_3['state'], constants.RequestState.WAITING)
        request_4 = get_request_by_did(self.scope, name4, self.dest_rse_2)
        assert_equal(request_4['state'], constants.RequestState.WAITING)
