...
from rucio.core.request import release_all_waiting_requests, queue_requests, get_request_by_did, release_waiting_requests_per_free_volumn, release_waiting_requests_grouped_fifo, release_waiting_requests_fifo
...
class TestRequestCore(object):
    ...
    def test_release_waiting_requests_per_free_volumn(self):
        """ REQUEST (CORE): release waiting requests that fit grouped in available volumn."""
        # release requests that fit in available volumn with respect to already submitted transfers
        name1 = generate_uuid()
        add_replica(self.source_rse, self.scope, name1, 1, self.account)
        name2 = generate_uuid()
        add_replica(self.source_rse, self.scope, name2, 1, self.account)
        name3 = generate_uuid()
        add_replica(self.source_rse, self.scope, name3, 1, self.account)
        ...
        max_transfer_volumn = 10
        set_rse_transfer_limits(self.dest_rse, 'all_activities', max_volumn=max_transfer_volumn, max_transfers=1, session=self.db_session)
        requests = [{
        ...
        release_waiting_requests_per_free_volumn(self.dest_rse, max_transfer_volumn=max_transfer_volumn, session=self.db_session)
        # releaed because requested first and small enough
        ...
        # release requests that fit in available volumn -> also release grouped
        self.db_session.query(models.Request).delete()
        self.db_session.commit()
        name1 = generate_uuid()
        add_replica(self.dest_rse, self.scope, name1, 1, self.account)
        ...
        queue_requests(requests, session=self.db_session)
        release_waiting_requests_per_free_volumn(self.dest_rse, max_transfer_volumn=max_transfer_volumn, session=self.db_session)
        # waiting because no available volumn
        request = get_request_by_did(self.scope, name1, self.dest_rse, session=self.db_session)
        assert_equal(request['state'], constants.RequestState.WAITING)

        # TODO test max_volumn 0 nichts releasen ohne request vorher und mit attachments
    ...
    def test_release_waiting_requests_grouped_fifo(self):
        ...
        set_rse_transfer_limits(self.dest_rse, self.all_activities, max_volumn=0, max_transfers=1, session=self.db_session)

        # one request with an unattached DID - one request should be released
        self.db_session.query(models.Request).delete()
        self.db_session.commit()
        name = generate_uuid()
        add_replica(self.source_rse, self.scope, name, 1, self.account)
        requests = [{
        ...
        add_replica(self.source_rse, self.scope, name, 1, self.account, session=self.db_session)
        ...
        add_replica(self.source_rse, self.scope, name5, 1, self.account, session=self.db_session)
        ...
        request_3 = get_request_by_did(self.scope, name3, self.dest_rse, session=self.db_session)
        ...
        request_5 = get_request_by_did(self.scope, name5, self.dest_rse, session=self.db_session)
        ...
        set_rse_transfer_limits(self.dest_rse, self.all_activities, max_volumn=10, max_transfers=1, session=self.db_session)
        requests = [{
            'source_rse_id': self.source_rse_id,
            'dest_rse_id': self.dest_rse_id,
            'request_type': constants.RequestType.TRANSFER,
            ...
            'name': name3,
            'bytes': 3,
            'requested_at': datetime.now().replace(year=2021),  # requested after the request below but small enough for max_volumn check
            'rule_id': generate_uuid(),
            'scope': self.scope,
            'retry_count': 1,
            'attributes': {
        ...
        request_2 = get_request_by_did(self.scope, name2, self.dest_rse, session=self.db_session)
        assert_equal(request_2['state'], constants.RequestState.QUEUED)
        # released because of available volumn
        request_3 = get_request_by_did(self.scope, name3, self.dest_rse, session=self.db_session)
        ...
        attach_dids(self.scope, dataset_1_name, [{'name': name1, 'scope': self.scope}], self.account, session=self.db_session)
        attach_dids(self.scope, dataset_1_name, [{'name': name2, 'scope': self.scope}], self.account, session=self.db_session)
        set_rse_transfer_limits(self.dest_rse, self.all_activities, max_volumn=5, max_transfers=1, session=self.db_session)
        request = models.Request(dest_rse_id=self.dest_rse_id, bytes=2, activity=self.all_activities, state=constants.RequestState.SUBMITTED)
        request.save(session=self.db_session)
        requests = [{
            'dest_rse_id': self.dest_rse_id,
        ...
        request_2 = get_request_by_did(self.scope, name2, self.dest_rse, session=self.db_session)
        assert_equal(request_2['state'], constants.RequestState.QUEUED)
        # still waiting because there is no free volumn after releasing the two requests above
        request_3 = get_request_by_did(self.scope, name3, self.dest_rse, session=self.db_session)
        assert_equal(request_3['state'], constants.RequestState.WAITING)
        request_4 = get_request_by_did(self.scope, name4, self.dest_rse, session=self.db_session)
        assert_equal(request_4['state'], constants.RequestState.WAITING)
    ...
    def test_release_waiting_requests_fifo(self):
        ...
        queue_requests(requests, session=self.db_session)
        release_waiting_requests_fifo(self.dest_rse, count=1, session=self.db_session)
        request = get_request_by_did(self.scope, name1, self.dest_rse, session=self.db_session)
        assert_equal(request['state'], constants.RequestState.QUEUED)
        request2 = get_request_by_did(self.scope, name2, self.dest_rse, session=self.db_session)
        ...
        queue_requests(requests, session=self.db_session)
        release_waiting_requests_fifo(self.dest_rse, count=2, account=self.account, activity=self.user_activity, session=self.db_session)
        request = get_request_by_did(self.scope, name1, self.dest_rse, session=self.db_session)
        assert_equal(request['state'], constants.RequestState.QUEUED)
        request = get_request_by_did(self.scope, name2, self.dest_rse, session=self.db_session)
        assert_equal(request['state'], constants.RequestState.WAITING)
        request = get_request_by_did(self.scope, name3, self.dest_rse, session=self.db_session)
        assert_equal(request['state'], constants.RequestState.WAITING)
        request = get_request_by_did(self.scope, name4, self.dest_rse, session=self.db_session)
        assert_equal(request['state'], constants.RequestState.QUEUED)
    ...
    def test_release_waiting_requests_all(self):
        ...
        queue_requests(requests, session=self.db_session)
        release_all_waiting_requests(self.dest_rse, session=self.db_session)
        request = get_request_by_did(self.scope, name1, self.dest_rse, session=self.db_session)
        assert_equal(request['state'], constants.RequestState.QUEUED)
        request = get_request_by_did(self.scope, name2, self.dest_rse, session=self.db_session)
        assert_equal(request['state'], constants.RequestState.QUEUED)
