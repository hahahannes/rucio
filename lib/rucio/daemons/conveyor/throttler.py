    ...
    try:
        ...
        if direction == 'destination' or direction == 'source':
            for rse_id in result_dict:
                ...
                if availability & 2:  # dest_rse is not blacklisted for write TODO read direction check
                    if all_activities:
    ...
    if threshold is not None and transfer + waiting > threshold:
        ...
        if transfer < 0.8 * threshold:
            ...
            if strategy == 'grouped_fifo':
                ...
                release_waiting_requests_grouped_fifo(rse_name, rse_id=rse_id, count=to_be_released, direction=direction, volume=volume, deadline=deadline)
            elif strategy == 'fifo':
                release_waiting_requests_fifo(rse_name, rse_id=rse_id, count=to_be_released, direction=direction)
        else:
        ...
        delete_rse_transfer_limits(rse=None, activity='all_activities', rse_id=rse_id)
        release_all_waiting_requests(rse=None, rse_id=rse_id, direction=direction)
        ...
        for activity in stats['activities']:
            ...
            if waiting:
                ...
                if threshold is None:
                    ...
                    delete_rse_transfer_limits(rse=None, activity=activity, rse_id=rse_id)
                    release_all_waiting_requests(rse=None, activity=activity, rse_id=rse_id, direction=direction)
                    ...
                    set_rse_transfer_limits(rse=None, activity=activity, rse_id=rse_id, max_transfers=threshold, transfers=transfer, waitings=waiting)
                    ...
                    if transfer < 0.8 * threshold:
                        ...
                        for account in accounts:
                            if nr_accounts == 1:
                                ...
                                release_waiting_requests_fifo(rse=None, activity=activity, rse_id=rse_id, account=account, count=to_release, direction=direction)
                                ...
                                release_waiting_requests_fifo(rse=None, activity=activity, rse_id=rse_id, account=account, count=accounts[account]['waiting'], direction=direction)
                                ...
                                release_waiting_requests_fifo(rse=None, activity=activity, rse_id=rse_id, account=account, count=to_release_per_account, direction=direction)
