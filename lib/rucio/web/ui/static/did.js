...
handle_file = function(scope, name) {
    ...
    load_did_tracking(scope, name);
};

load_did_tracking = function(scope, name) {
    ...
    var data = [{
        ...
        data: [{
            ...
            to: new Date([2019, 1, 20]),
            customClass: 'red-interval'
        }, {
            label: 'Replicating',
            type: TimelineChart.TYPE.INTERVAL,
    ...
    promises.push(new Promise(function(resolve, reject) {
        r.did_get_rules({'scope': scope, 'name': name, 'success': function(rules) {
            rules.forEach(function(rule) {
                ...
                table_data.push({'timestamp': new Date(rule.created_at), 'detail': 'Rule ' + rule.id + ' created<br>RSE expression: ' + rule.rse_expression})
            })
            resolve();
        }})
    }))
    promises.push(new Promise(function(resolve, reject) {
        r.get_did({'scope': scope, 'name': name, 'success': function(did) {
            if (did.expired_at) {
                ...
                table_data.push({'timestamp': new Date(rule.expired_at), 'detail': 'Expired'})
            }
            resolve();
        }})
    }))
    promises.push(new Promise(function(resolve, reject) {
        // list_Replicas ersetzen dann aber api erstellen um request_by_rule_id zu ermoeglichen momentan nicht da
        r.list_replicas({'scope': scope, 'name': name, 'success': function(replica) {
            for (var rse in replica[0]['rses']) {
                r.get_request_by_did({'rse': rse, 'scope': scope, 'name': name, 'success': function(request) {
                    if(request['request_type'] == 'TRANSFER') {
                        ...
                        table_data.push({'timestamp': new Date(request.created_at), 'detail': 'Transer request created<br>Destination RSE: ' + request.dest_rse_id})
                        data[2]['data'].push({
                            type: TimelineChart.TYPE.POINT,
                            at: new Date(request['submitted_at']),
                            desc: 'Submitted at: ' + new Date(request.submitted_at).toLocaleString()
                        })
                        table_data.push({'timestamp': new Date(request.submitted_at), 'detail': 'Transfer submitted'})
                        data[2]['data'].push({
                            type: TimelineChart.TYPE.POINT,
                            at: new Date(request['transfered_at']),
                            desc: 'Finished at: ' + new Date(request.transfered_at).toLocaleString()
                        })
                        table_data.push({'timestamp': new Date(request.transfered_at), 'detail': 'Transfer finished'})
                        data[1]['data'].push({
                            label: 'Replicating',
                            type: TimelineChart.TYPE.INTERVAL,
    ...
    Promise.all(promises).then(function() {
        const timeline = new TimelineChart(element, data, {
            ...
            tip: function(d) {
                rows = $('#tracking_table tr')
                for(var i=0; i<rows.length; i++) {
                    if($(rows[i]).find('th').text() == d.at.toLocaleString()) {
        ...
        for(var i=0; i < table_data.length; i++) {
            $('#tracking_table').append($('<tr><th>' + table_data[i]['timestamp'].toLocaleString() + '</th><td>' + table_data[i]['detail'] + '</td></tr>'));
        }
    })                                  
}
